 ...
 console.log(showCom)
    ...
    const Images =
      Array.isArray(availableProduct) && availableProduct.length > 0
        ? availableProduct.map(({ images }) => images[0])
        : "";
    const [Name, StyleName] =
      productData?.length > 0
        ? productData[currentIndex]?.name.split(" ")
        : ["", " "];

        const [NameCom, StyleNameCom] =
        combine_product.hasOwnProperty('name') 
          ? combine_product?.name.split(" ")
          : ["", " "];
    ...
    return (
      <div ref={ref} className="mt-10">
        <div>
          <div className="flex justify-start mx-4 items-center overflow-x-auto sticky top-[42px] z-40 border-b-[0.5px] border-[#0F93B1] bg-white ">
            {Array.isArray(Images)
              ? Images.map((src, index) => (
                  <div
                    className="cursor-pointer px-3 sm:px-4 lg:px-6 py-2 sm:py-3 flex-shrink-0 "
                    style={{
                      ...
                      marginRight: "-1px",
                      backgroundColor: index === currentIndex ? '#e4edfc' : 'white'
                    }}
                  >
                    <img
          ...
          {showDiv && dynamicURL !== "combine_product" ? (
            <div className=" px-2">
                ...
                <div>
                  <div className="flex items-center gap-2 px-3">
                    <img
                      className="w-5 h-5"
                      src={
                        productData[currentIndex]?.images[
                          productData[currentIndex]?.images?.length - 1
                        ]
                      }
                    />
                  ...
                  <h2 className=" text-xs md:text-lg  my-3 px-3">
                    {productData[currentIndex]?.short_description}
                  </h2>
                </div>
                <button>
                  {" "}
                  <ChevronUpIcon
                    style={{
                      transform: showDiv ? "rotate(180deg)" : "rotate(-360deg)",
              ...
              <table className="w-full border-separate border-spacing-y-4 table-fixed ">
                <thead className="sticky top-[150px] z-20 bg-white ">
                  <tr>
                    {singleProduct?.product_packages
                        ...
                        <th className="p-2 text-center flex-col flex-grow w-full">
                          <div className="flex flex-col items-center border-[0.8px] border-[#0F93B1] rounded-sm p-2 h-40 ">
                            <div className="transform -rotate-90 h-full flex flex-col justify-center ">
                              ...
                              <p className="text-[12px] md:text-base font-medium text-[#0F93B1] whitespace-nowrap w-auto">
                                100
                              </p>
                            </div>
                            <div className="bg-primary text-white px-3 py-1.5 rounded-md mt-2 text-xs w-auto">
                              Buy
                            </div>
                          </div>
                        </th>
                ...
                <tbody>
                  {singleProduct?.features?.map(
                    ({ feature, value,  product_packages }) => (
                      <>
                        {" "}
                        <tr>
              ...
              ({ short_description }, id) => (
                <div className=" px-2">
                  ...
                  <table  className="w-full border-separate border-spacing-y-4 table-fixed ">
                    <thead className="sticky top-[150px] z-20 bg-white ">
                      <tr>
                        {combine_product?.product_packages?.map(
                          ({ package: pack_ }, id) => (
                            <th
                              key={id}
                              className="p-2 text-center flex-col flex-grow w-full"
                            >
                              <div className="flex flex-col items-center border-[0.8px] border-[#0F93B1] rounded-sm p-2 h-40 ">
                                <div className="transform -rotate-90 h-full flex flex-col justify-center ">
                                  <h2 className="text-lg font-medium text-[#0F93B1] whitespace-nowrap w-auto">
                   ...
                   { showCom[id] && <tbody>
                      {combine_product.clone_products[id].features?.map(
                        ({ feature, value, product, product_packages }) => (
                          <>
                            {" "}
                            <tr>
                              <th colSpan={2} className=" px-2 mt-2 text-left">
